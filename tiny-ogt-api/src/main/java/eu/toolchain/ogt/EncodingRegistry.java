...
public class EncodingRegistry<Target> {
    ...
    public <Source> Stream<Encoder<Target, Source>> newEncoder(
        final EntityResolver resolver, final Type type, final EncodingFactory<Target> encoding
    ) {
        return encoders
            .stream()
    ...
    @FunctionalInterface
    interface EncoderBuilder<T, O> {
        Encoder<T, O> apply(
            EntityResolver resolver, Type type, EncodingFactory<T> encoding
        );
    }

    @FunctionalInterface
    interface DecoderBuilder<T, O> {
        Decoder<T, O> apply(
            EntityResolver resolver, Type type, EncodingFactory<T> encoding
        );
    }
